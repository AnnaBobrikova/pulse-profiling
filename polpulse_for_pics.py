...
t__e = np.arange(40.0, 202.0, 4.0) #check if now the ranges include the last element as well!
t__bb = np.arange(0.00015, 0.0031,0.00015)
tau__t = np.arange(0.5, 3.55, 0.1)

NEnergy = 150
NZenith = 9

I_mighty = ones((len(t__e), len(t__bb), len(tau__t), NEnergy, NZenith))
Q_mighty = ones((len(t__e), len(t__bb), len(tau__t), NEnergy, NZenith))
...
def reading_table():
    ...
    for i in t__e:
        ...
        print('With CompSlab_%s.fits still works' %(i))
        for ii in range(0,len(t__bb)):
            for iii in range(0,16):
                #print(ii, iii)
                #print((iii)*len(t__bb)+ii+1)
                science_data = hdu[(iii)*len(t__bb)+ii+1].data #watch out, mb new numbers are needed
                data = np.transpose(science_data.field(1))
                data2 = np.transpose(science_data.field(0))
                for kk in range(0, NEnergy):
                    for kkk in range(0,NZenith):
                        I_mighty[p, ii, iii, kk, kkk] = data[kk, kkk+1]#and here
                        Q_mighty[p, ii, iii, kk, kkk] = data2[kk, kkk+1]#and here
        p +=1
    ...
    for i in t__e:
        ...
        for ii in range(0,len(t__bb)):
            for iii in range(0,10):
                ...
                for kk in range(0, NEnergy):
                    for kkk in range(0,NZenith):
                        I_mighty[p, ii, iii+16, kk, kkk] = data[kk, kkk+1]#and here
                        Q_mighty[p, ii, iii+16, kk, kkk] = data2[kk, kkk+1]#and here
        p +=1
    p=0            
    for i in t__e:
        ...
        for ii in range(0,len(t__bb)):
            for iii in range(0,5):
                ...
                for kk in range(0, NEnergy):
                    for kkk in range(0,NZenith):
                        I_mighty[p, ii, iii+26, kk, kkk] = data[kk, kkk+1]#and here
                        Q_mighty[p, ii, iii+26, kk, kkk] = data2[kk, kkk+1]#and here
        p +=1



def compf(mass,eqrad,incl_deg,theta_deg,rho_deg,pol,spherical=False,antipodal=False,spath="pulse_test_00",savePulse=False,nene=150, te=102.0, tbb=0.001, taut = 1.0):
	PulsName=spath
	...
	NZenith = 9 # number of propagation zenith angles (z) [0,pi]
	...
	if Spectrum=='simplThomson' : # Initializing Stokes vectors arrays, computing zeroth scattering 
		print('sorry, we dont do that anymore, I need to find missing variables')
	...
	if Spectrum=='FromFile' :
		...
		I_2D = ones((NEnergy, NZenith))
		P_2D = ones((NEnergy, NZenith)) #these two will contain the interpolation results of log(I) and PD (Q/I) for specific values of Te, Tbb and TauT (given by the Task, one combination at a time) for all Es and mus.
		
		xx = np.linspace(40.0, 200.0, 41)
		...
		for kk in range(0, NEnergy):
			for kkk in range(0, NZenith):
				for i in range(0, len(t__e)):
					for ii in range(0, len(t__bb)):
						for iii in range(0, len(tau__t)):
							I_3D[i, ii, iii] = np.log(I_mighty[i, ii, iii, kk, kkk])
				...
				Intensity[kk,kkk,0]=np.exp(I_2D[kk,kkk])
				Intensity[kk,kkk,1] = P_2D[kk,kkk]*Intensity[kk,kkk,0]
				print(kk,kkk)
