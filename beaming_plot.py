...
if(plot_spec=="ene"):
	...
	ax.plot(keV[:],keV[:]*Ibeam[:,ide0]*e_Erg,"-",color=cols[0],label="$\mu$ = 0.0")
	ax.plot(keV[:],keV[:]*Ibeam[:,ide1]*e_Erg,"-",color=cols[1],label="$\mu$ = 0.2")
	ax.plot(keV[:],keV[:]*Ibeam[:,ide2]*e_Erg,"-",color=cols[2],label="$\mu$ = 0.4")
	ax.plot(keV[:],keV[:]*Ibeam[:,ide3]*e_Erg,"-",color=cols[3],label="$\mu$ = 0.6")
	ax.plot(keV[:],keV[:]*Ibeam[:,ide4]*e_Erg,"-",color=cols[4],label="$\mu$ = 0.8")
	ax.plot(keV[:],keV[:]*Ibeam[:,ide5]*e_Erg,"-",color=cols[5],label="$\mu$ = 1.0")
...
if(plot_spec=="ene"):
	...
	ax.set_ylabel('$E$ $I_{E}$',fontsize=lbfontsz) #UNITS LOW-KEY CHECKED HERE
