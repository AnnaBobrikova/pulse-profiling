...
#import:
from numpy import linspace, logspace, zeros, fromfile
import matplotlib
matplotlib.use('agg')
from numpy import pi, exp, log, sqrt, sin, cos, arccos, arctan2
from numpy import absolute, sign, argmin
...
def compf(mass,eqrad,incl_deg,theta_deg,rho_deg,pol,ekev,ph,spherical=False,antipodal=False,spath="pulse_test_00",savePulse=False,atmos_path="model/atmos_thom/"):
	...
	if Spectrum=='simplThomson' : # Initializing Stokes vectors arrays, computing zeroth scattering 
		print('not anymore, we dont do simple thomson')

	if Spectrum=='FromFile' :
		from astropy.io import fits
		import numpy as np
		hdu = fits.open('CompSlab_Int3D_testgrid.fits', mode='update')
		science_data1 = hdu[1].data
		QQ = science_data1.field(0)
		II = science_data1.field(1)

		QQ = np.transpose(QQ)
		II = np.transpose(II)

		Intensity=zeros((NEnergy,NZenith,2))
		for EE in range(0,NEnergy):  
			for d in range(0,NZenith):  
				Intensity[EE,d,0]=II[EE,d]
				Intensity[EE,d,1]=QQ[EE,d]
