...
from polpulse_for_pics import compf, reading_table
		...
		for i in range(len(incls)):
			for it in range(len(thetas)):
				for ai in range(len(antipds)):
					...
					#	continue
					Flux = compf(mass,rad,incl,theta,rho,pol,spherical=False,antipodal=antpd,spath=spath,savePulse=True, te=97.8, tbb=0.00196, taut=1.0)
else:
	Flux = compf(mass,rad,incl,theta,rho,pol,spherical=False,antipodal=False,spath="pulses/test/pulse_testX",savePulse=True, te=97.8, tbb=0.00196, taut=1.0) 
