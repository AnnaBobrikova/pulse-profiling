...
def savePyt(temppytI,temppytQ,temppytU,Filename_new="pulse_test2",Filename_old="pulse_test"):
	...
	Flux1 = fromfile(Flux)
	phase = fromfile(phi)
	NEnergy =150  #n of energy bins in bin
...
def readPyt(Filename="pulse_test"):
	...
	Flux1 = fromfile(Flux)
	phi1 = fromfile(phi)
	NEnergy =150 # 500 #50 # n of energy bins in bin
...
def plot2():
	...
	for ene in (find_idx(keV,2),find_idx(keV,5),find_idx(keV,8)):
		...
		for ip in range(0,Nphase-1):
			if(PA1[ip]>90):PA1[ip]=PA1[ip]-90
			else: PA1[ip]=PA1[ip]+90
			if(PA2[ip]>90):PA2[ip]=PA2[ip]-90
			else: PA2[ip]=PA2[ip]+90			
			if(PA3[ip]>90):PA3[ip]=PA3[ip]-90
			elif(PA3[ip]>0): PA3[ip]=PA3[ip]+90
		nI1 = I1[:,ene]/np.max(I1[:,ene])
		#nI2 = I2[:,ene]/np.max(I2[:,ene])
		#nI3 = I3[:,ene]/np.max(I3[:,ene])
		nI2 = I2[:,ene]/np.max(I1[:,ene])
	...
	for ene in (find_idx(keV,2),find_idx(keV,5),find_idx(keV,8)):
		...
		if(it == 1):
			...
			ax2b.plot(phase, Q3[:,ene]/I3[:,ene], '.', label="spot2", color=col3,markersize=msz)
			ax2b.set_xlim(0., 1.)
			#ax2b.set_ylabel("$Q/I$")
			...
			ax3b.plot(phase, U3[:,ene]/I3[:,ene], '.', label="spot2", color=col3,markersize=msz) #HERE I changed U3perI for U3/I3
			ax3b.set_xlim(0., 1.)
			#ax3b.set_ylabel("$U/I$")
			ax3b.set_xlabel("Phase $\phi/2\pi$")
