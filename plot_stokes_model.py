...
def plot2():
	...
	for ene in (find_idx(keV,2),find_idx(keV,5),find_idx(keV,8)):
		...
		if(it == 1):
			...
			ax2b.plot(phase, Q3[:,ene]/I3[:,ene], '.', label="spot2", color=col3,markersize=msz) #HERE WE CHANGE TO NOT GET ZEROS
			ax2b.set_xlim(0., 1.)
			#ax2b.set_ylabel("$Q/I$")
			...
			ax3b.plot(phase, U3[:,ene]/I3[:,ene], '.', label="spot2", color=col3,markersize=msz) #HERE AS WELL
			ax3b.set_xlim(0., 1.)
			#ax3b.set_ylabel("$U/I$")
			ax3b.set_xlabel("Phase $\phi/2\pi$")
